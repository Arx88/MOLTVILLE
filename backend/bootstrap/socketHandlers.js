...
export function registerRealtimeSocketHandlers({
  ...
  const truncateLogValue = (value, max = 240) => {
  ...
  socket.on('agent:connect', async (data) => {
    ...
    try {
      ...
      if (agentAutonomyEngine) {
        agentAutonomyEngine.onAgentConnected(agent.id);
      }
    } catch (error) {
  ...
  socket.on('agent:profile', (data = {}) => {
    ...
    if (updated) {
      const cognition = data?.cognition && typeof data.cognition === 'object' && !Array.isArray(data.cognition)
        ? data.cognition
        : {};
      const thinkPayload = {
        agentId: socket.agentId,
        agentName: updated.name || null,
        internalThought: truncateLogValue(cognition.internalThought, 320),
        externalIntent: truncateLogValue(cognition.externalIntent, 320),
        externalSpeech: truncateLogValue(cognition.externalSpeech, 320),
        plan: truncateLogValue(data?.plan, 320)
      };
      if (thinkPayload.internalThought || thinkPayload.externalIntent || thinkPayload.externalSpeech || thinkPayload.plan) {
        logger.info('AGENT_THINK', thinkPayload);
      }
      const baseAgents = moltbotRegistry.getAllAgents();
      const enriched = baseAgents.map(agent => ({
        ...agent,
  ...
  socket.on('agent:speak', async (data) => {
    ...
    try {
      ...
      const safePosition = position && Number.isFinite(position.x) && Number.isFinite(position.y)
      ...
      logger.info('AGENT_SAY', {
        ...
        utterance: truncateLogValue(message, 320),
        nearbyAgents: Math.max(0, nearbyAgents.length - 1),
  ...
  socket.on('agent:action', async (data) => {
    ...
    try {
      ...
      const actingAgent = moltbotRegistry.getAgent(socket.agentId);
      logger.info('AGENT_DO', {
        agentId: socket.agentId,
        agentName: actingAgent?.name || null,
        actionType,
        target: truncateLogValue(target, 240),
        params: truncateLogValue(params, 280)
      });
    } catch (error) {
  ...
  socket.on('disconnect', () => {
    if (socket.agentId) {
      ...
      if (agent) {
        ...
        if (agentAutonomyEngine) {
          agentAutonomyEngine.onAgentDisconnected(socket.agentId);
        }
